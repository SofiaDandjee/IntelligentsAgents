...
public class PlannerSingleTask {

    List<Vehicle> vehicles;
    List<Task> tasks;

    PlannerSingleTask(List<Vehicle> v, List<Task> t) {
        this.vehicles = v;
        this.tasks = t;
    }
    StateSingleTask selectInitialSolutionNaive() {
        ...
        return new StateSingleTask(vehicleToTask, taskToTask, taskToTime, taskToVehicle);
    }

    private StateSingleTask selectInitialSolutionBetter() {
    ...
    private List<StateSingleTask> chooseNeighbours(StateSingleTask s) {
        List<StateSingleTask> neighbours = new ArrayList<>();
        ...
        for (Vehicle vj : vehicles) {
            ...
            if (v != vj) {
                ...
                if (t.weight <= vj.capacity()) {
                    ...
                    StateSingleTask s1 = changingVehicle(s, v, vj);
                    neighbours.add(s1);
                }
            }
        ...
        if (n >= 2) {
            for (int id1 = 1; id1 <= n-1; ++id1) {
                for (int id2 = id1+1; id2 <= n; ++id2) {
                    StateSingleTask s1 = changingTaskOrder(s, v, id1, id2);
                    neighbours.add(s1);
                }
            }
        }
...
//    private void updateTime(StateSingleTask s, Vehicle v) {
...
//    }
    ...
    private StateSingleTask changingVehicle(StateSingleTask s, Vehicle v1, Vehicle v2) {

        StateSingleTask s1 = new StateSingleTask(s);
...
//        updateTime(s1, v2);
    ...
    private StateSingleTask changingTaskOrder(StateSingleTask s, Vehicle v, int id1, int id2) {
        StateSingleTask s1 = new StateSingleTask(s);
        Task task1Prev;
        Task t1;
...
//        updateTime(s1, v);
    ...
    private StateSingleTask localChoice(List<StateSingleTask> N, StateSingleTask sOld) {
        double minCost = Double.POSITIVE_INFINITY;
        double cost;
//        StateSingleTask choice = null;
        List<StateSingleTask> bestChoices = new ArrayList<>();
        double probThreshold = 0.4;
        for (StateSingleTask neighbour : N) {
    ...
    public StateSingleTask SLS() {
        StateSingleTask s = selectInitialSolutionNaive();
        int maxIter = 10000;
        int iter = 0;
        double bestCost = s.getCost();
        StateSingleTask bestStateSingleTask = s;
        do {
            StateSingleTask sOld = s;
            List <StateSingleTask> neighbours = chooseNeighbours(sOld);
            s = localChoice(neighbours, sOld);
            if (s.getCost() < bestCost) {
                bestCost = s.getCost();
                bestStateSingleTask = s;
                System.out.println("Better solution found!");
            }
            ++iter;

        } while (iter < maxIter);
        return bestStateSingleTask;
    }
